...
<!--</a>-->
{{if is_completed == 3}}
 <a title="<?php echo __('messages.common.cancel'); ?>" class="btn btn-icon btn-bg-light btn-active-color-danger btn-sm">
     <i class="fas fa-calendar-times text-danger"></i>
  </a>
 {{else}}
  <a title="<?php echo __('messages.common.cancel'); ?>" data-id="{{:id}}" class="cancel-appointment btn btn-icon btn-bg-light btn-active-color-danger btn-sm {{:is_completed == 1 ? "d-none"  : "" }}">
     <i class="far fa-calendar-times {{:is_completed == 1 ? "text-danger"  : "" }}"></i>
  </a>
 {{/if}}
 {{if !isPatient}}
{{if is_completed == 1 || is_completed == 3}}
     <a title="<?php echo __('messages.common.confirm'); ?>" class="btn btn-icon btn-bg-light btn-active-color-success btn-sm {{:is_completed == 3 ? "d-none"  : "" }}">
        <i class="fas fa-calendar-check text-success {{:is_completed == 3 ? "d-none"  : ""}}"></i>
     </a>
{{/if}}
    {{if is_completed == 0}}
     <a title="<?php echo __('messages.common.confirm'); ?>" data-id={{:id}}" class="status btn btn-icon btn-bg-light btn-active-color-success btn-sm">
